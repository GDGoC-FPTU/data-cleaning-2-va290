...
def clean_data(input_file, output_file):
    ...
    for item in data:
        ...
        # Mask the 'email' field using the mask_email function
        if 'email' in item:
            item['email'] = mask_email(item['email'])

        # 5. Category Normalization: Fill missing/empty category (Improvement)
        # An empty string or missing 'category' becomes a safe default
        # instead of dropping an otherwise valid record.
